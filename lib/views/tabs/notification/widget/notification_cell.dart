...
class CommentNoticationCell extends StatelessWidget {
  final KSNotification notification;
  const CommentNoticationCell({Key? key, required this.notification}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Material(
      color: Theme.of(context).primaryColor,
      child: InkWell(
        onTap: () => launchScreen(context, DetailScreen.tag, arguments: {'postId': notification.post}),
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0),
          child: Row(
            children: [
...
class LikeNoticationCell extends StatelessWidget {
  final KSNotification notification;
  const LikeNoticationCell({Key? key, required this.notification}) : super(key: key);
